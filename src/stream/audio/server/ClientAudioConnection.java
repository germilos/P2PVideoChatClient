...
import chat.client.audio.AudioServer;
 ...
 */
public class ClientAudioConnection extends Thread {

    private AudioServer serv; //instance of server, needed to put messages in the server's broadcast queue
    ...
    private ArrayList<SoundMessage> toSend = new ArrayList<SoundMessage>(); //queue of messages to be sent to the client
    ...
    public ClientAudioConnection(AudioServer serv, Socket s) {
        this.serv = serv;
        this.s = s;
    ...
    public void addToQueue(SoundMessage m) { //add a message to send to the client
        try {
            toSend.add(m);
        } catch (Throwable t) {
        ...
        for (;;) {
            try {
                if (s.getInputStream().available() > 0) { //we got something from the client
                    SoundMessage toBroadcast = (SoundMessage) in.readObject(); //read data from client
                ...
                try {
                    if (!toSend.isEmpty()) {
                        SoundMessage toClient = toSend.get(0); //we got something to send to the client
